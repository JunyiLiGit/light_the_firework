  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/chaiseLounge.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load chaiseLounge.mp3")+Mix_GetError();

  startMusic();
  sounds.push_back( Mix_LoadWAV("sound/leaf.wav") );
  sounds.push_back( Mix_LoadWAV("sound/pond.wav") );
  sounds.push_back( Mix_LoadWAV("sound/evilrock.wav") );
  sounds.push_back( Mix_LoadWAV("sound/firework.wav") );
  sounds.push_back( Mix_LoadWAV("sound/umbrella.wav") );
  sounds.push_back( Mix_LoadWAV("sound/rocksmall.wav") );
  sounds.push_back( Mix_LoadWAV("sound/rainshoot.wav") );
  sounds.push_back( Mix_LoadWAV("sound/rain.wav") );
  sounds.push_back( Mix_LoadWAV("sound/fireballshoot.wav") );
